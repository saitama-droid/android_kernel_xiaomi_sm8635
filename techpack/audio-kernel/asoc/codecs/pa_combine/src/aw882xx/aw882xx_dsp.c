...
static int aw_send_afe_rx_module_enable(void *buf, int size) { return 0; }
static int aw_send_afe_tx_module_enable(void *buf, int size) { return 0; }
static int afe_get_topology(int port_id) { return 0; }
...
static int aw_adm_param_enable(int port_id, int module_id, int param_id,
  ...
  aw_pr_dbg("port_id %d, module_id 0x%x, copp_idx 0x%x, enable %d", port_id,
            module_id, copp_idx, enable);
...
static void aw_dsp_get_check_sum(int pos, void *data, int size,
                                 int *check_sum) {
  int i = 0;
  int sum_data = 0;
...
static int aw_mtk_write_data_to_dsp(int param_id, void *data, int size) {
  ...
  ret = mtk_spk_send_ipi_buf_to_dsp(dsp_data, sizeof(aw_dsp_msg_t) + size);
  if (ret < 0) {
...
static int aw_read_msg_from_dsp_v_1_0_0_0(struct aw_device *aw_dev,
  ...
  aw_dsp_get_check_sum(sizeof(int32_t), dsp_msg, sizeof(aw_msg_hdr_t) + len,
                       &sum_data);

  if (sum_data != read_hdr->checksum) {
    aw_pr_err("aw_dsp_msg check sum error!");
    aw_pr_err("read_hdr->checksum=%d sum_data=%d", read_hdr->checksum,
              sum_data);
    ret = -EINVAL;
    goto read_msg_error;
  }
...
int aw882xx_dsp_read_dsp_msg(struct aw_device *aw_dev, uint32_t msg_id,
  ...
  if (aw_dev->channel < AW_DEV_CH_TERT_L) {
    ...
    ret = aw_read_msg_from_dsp_v_1_0_0_0(aw_dev, msg_id, data_ptr, size,
...
int aw882xx_dsp_write_dsp_msg(struct aw_device *aw_dev, uint32_t msg_id,
  ...
  if (aw_dev->channel < AW_DEV_CH_TERT_L) {
    ...
    ret = aw_write_msg_to_dsp_v_1_0_0_0(aw_dev, msg_id, data_ptr, size,
                                        AW_DSP_CHANNEL_DEFAULT_NUM);
  }
...
int aw882xx_dsp_read_te(struct aw_device *aw_dev, int32_t *te) {
  ...
  ret = aw882xx_dsp_read_dsp_msg(aw_dev, AW_MSG_ID_SPK_STATUS, (char *)data,
                                 sizeof(int32_t) * 8);
  if (ret) {
...
int aw882xx_get_algo_version(struct aw_device *aw_dev, char *algo_ver_buf) {
  ...
  ret = aw882xx_dsp_read_dsp_msg(aw_dev, AW_MSG_ID_VERSION, (char *)&algo_ver,
                                 sizeof(uint32_t));
  if ((ret < 0) || (algo_ver == 0)) {
    ret = aw882xx_dsp_read_dsp_msg(aw_dev, AW_MSG_ID_VERSION_NEW, algo_ver_buf,
                                   ALGO_VERSION_MAX);
    if (ret < 0)
      return ret;
  } else {
...
void aw882xx_device_parse_topo_id_dt(struct aw_device *aw_dev) {
  ...
  aw_dev_info(aw_dev->dev, "tx-topo-id: 0x%x, rx-topo-id: 0x%x", g_tx_topo_id,
              g_rx_topo_id);
}
...
void aw882xx_device_parse_port_id_dt(struct aw_device *aw_dev) {
  ...
  aw_dev_info(aw_dev->dev, "tx-port-id: 0x%x, rx-port-id: 0x%x", g_tx_port_id,
              g_rx_port_id);
}
