...
static int
aw_monitor_trace_data_from_table(struct aw_device *aw_dev,
                                 struct aw_table_info table_info,
                                 struct aw_monitor_trace *data_trace) {
  int i;
...
static int aw_monitor_get_data_from_table(struct aw_device *aw_dev,
  ...
  if (monitor->first_entry == AW_FIRST_ENTRY) {
    return aw_monitor_first_get_data_form_table(aw_dev, table_info, data_trace);
  } else {
    ...
    data_trace->sum_val = ((int32_t)aplha * data_trace->sum_val +
                          ...
                          1000;
    return aw_monitor_trace_data_from_table(aw_dev, table_info, data_trace);
  }
...
static int aw_monitor_get_data(struct aw_device *aw_dev) {
  ...
  if (monitor_cfg->vol_switch) {
    ret = aw_monitor_get_data_from_table(aw_dev, monitor_cfg->vol_info,
                                         vol_trace, monitor_cfg->vol_aplha);
    if (ret < 0)
      return ret;
  } else {
  ...
  if (monitor_cfg->temp_switch) {
    ret = aw_monitor_get_data_from_table(aw_dev, monitor_cfg->temp_info,
                                         temp_trace, monitor_cfg->temp_aplha);
    if (ret < 0)
      return ret;
  } else {
...
static void aw_monitor_get_cfg(struct aw_device *aw_dev,
  ...
  if (temp_data->ipeak == IPEAK_NONE && vol_data->ipeak == IPEAK_NONE) {
    ...
    if (monitor->monitor_cfg.logic_switch == AW_MON_LOGIC_OR) {
      set_table->ipeak = (temp_data->ipeak < vol_data->ipeak ? temp_data->ipeak
                                                             : vol_data->ipeak);
      set_table->gain =
          (temp_data->gain < vol_data->gain ? vol_data->gain : temp_data->gain);
      set_table->vmax =
          (temp_data->vmax < vol_data->vmax ? vol_data->vmax : temp_data->vmax);
    } else {
      set_table->ipeak =
          (temp_data->ipeak < vol_data->ipeak ? vol_data->ipeak
                                              : temp_data->ipeak);
      set_table->gain =
          (temp_data->gain < vol_data->gain ? temp_data->gain : vol_data->gain);
      set_table->vmax =
          (temp_data->vmax < vol_data->vmax ? temp_data->vmax : vol_data->vmax);
    }
  }
}
...
static void aw_monitor_set_ipeak(struct aw_device *aw_dev, uint16_t ipeak) {
  ...
  if (read_reg_val == ipeak) {
    aw_dev_dbg(aw_dev->dev, "ipeak = 0x%x, no change", read_reg_val);
    return;
  }
  ...
  aw_dev_info(aw_dev->dev, "set reg val = 0x%x, ipeak = 0x%x", reg_val, ipeak);
}
...
static void aw_monitor_work_func(struct work_struct *work) {
  struct aw_device *aw_dev =
      container_of(work, struct aw_device, monitor_desc.delay_work.work);
  ...
  if ((monitor_cfg->monitor_status == AW_MON_CFG_OK) &&
    ...
    if (!aw_get_hmute(aw_dev)) {
      ...
      if (monitor->monitor_mode == AW_MON_KERNEL_MODE) {
        queue_delayed_work(aw882xx->work_queue, &monitor->delay_work,
...
static int aw_monitor_check_fw(struct aw_device *aw_dev, uint8_t *data,
  ...
  if (temp_size > data_len) {
    aw_dev_err(aw_dev->dev, "temp_size:[%d] overflow file size[%d]!", temp_size,
               data_len);
    return -ENOMEM;
  }
  ...
  if (vol_size > data_len) {
    aw_dev_err(aw_dev->dev, "vol_size:[%d] overflow file size[%d]!", vol_size,
               data_len);
    return -ENOMEM;
  }
...
static void aw_monitor_write_data_to_table(struct aw_device *aw_dev,
  ...
  for (i = 0; i < table_info->table_num; i++)
    aw_dev_info(aw_dev->dev,
                ...
                table_info->aw_table[i].max_val, table_info->aw_table[i].ipeak,
                table_info->aw_table[i].gain, table_info->aw_table[i].vmax);
}

static int aw_monitor_parse_temp_data(struct aw_device *aw_dev, uint8_t *data) {
  struct aw_monitor_hdr *monitor_hdr = (struct aw_monitor_hdr *)data;
  struct aw_table_info *temp_info = &aw_dev->monitor_desc.monitor_cfg.temp_info;
  ...
  temp_info->aw_table = devm_kzalloc(
      aw_dev->dev, (monitor_hdr->temp_num * AW_TABLE_SIZE), GFP_KERNEL);
  if (temp_info->aw_table == NULL)
    return -ENOMEM;
...
static int aw_monitor_parse_vol_data(struct aw_device *aw_dev, uint8_t *data) {
  struct aw_monitor_hdr *monitor_hdr = (struct aw_monitor_hdr *)data;
  struct aw_table_info *vol_info = &aw_dev->monitor_desc.monitor_cfg.vol_info;
  ...
  vol_info->aw_table = devm_kzalloc(
      aw_dev->dev, (monitor_hdr->vol_num * AW_TABLE_SIZE), GFP_KERNEL);
  if (vol_info->aw_table == NULL)
    return -ENOMEM;
...
static int aw_monitor_real_time_update_monitor(struct aw_device *aw_dev) {
  ...
  aw_monitor_cnt =
      devm_kzalloc(aw_dev->dev, cont->size + sizeof(uint32_t), GFP_KERNEL);
  ...
  ret = aw882xx_monitor_parse_fw(&aw_dev->monitor_desc, aw_monitor_cnt->data,
                                 aw_monitor_cnt->len);
  if (ret < 0)
...
static struct attribute *aw_monitor_attr[] = {
    &dev_attr_monitor.attr, &dev_attr_monitor_update.attr,
#ifdef AW_DEBUG
    &dev_attr_vol.attr, &dev_attr_temp.attr,
#endif
    NULL};
