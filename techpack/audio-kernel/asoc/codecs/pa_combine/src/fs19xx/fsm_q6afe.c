...
int fsm_afe_get_rx_port(void) { return fsm_rx_port; }

int fsm_afe_get_tx_port(void) { return fsm_tx_port; }
...
static int fsm_afe_send_inband(struct fsm_afe *afe, void *buf,
                               uint32_t length) {
  int ret;

  pr_debug("port:%x, module:%x, param:0x%x, length:%d", afe->port_id,
           afe->module_id, afe->param_id, length);
  ...
  if (ret) {
    pr_err("sent packet for port %d failed with code %d", afe->port_id, ret);
    goto exit;
  }
  pr_debug("sent packet with param id 0x%08x to module 0x%08x", afe->param_id,
           afe->module_id);
  if (!afe->op_set) {
...
static int fsm_afe_map_rtac_block(struct rtac_cal_block_data *fsm_cal) {
  ...
  if (fsm_cal->map_data.dma_buf == NULL) {
    ...
    ret = msm_audio_ion_alloc(
        &(fsm_cal->map_data.dma_buf), fsm_cal->map_data.map_size,
        &(fsm_cal->cal_data.paddr), &len, &(fsm_cal->cal_data.kvaddr));
    if (ret < 0) {
  ...
  if (fsm_cal->map_data.ion_handle == NULL) {
    fsm_cal->map_data.map_size = SZ_4K;
    ret = msm_audio_ion_alloc(
        "fsm_cal", &(fsm_cal->map_data.ion_client),
...
int fsm_afe_send_apr(struct fsm_afe *afe, void *buf, uint32_t length) {
  ...
  pr_info("port:%x, module:%x, param:0x%x, length:%d", afe->port_id,
          afe->module_id, afe->param_id, length);
  ...
  afe->mem_hdr.data_payload_addr_lsw = lower_32_bits(cal_data->cal_data.paddr);
  ...
  pr_debug("sent packet with param 0x%08x to module 0x%08x", afe->param_id,
           afe->module_id);
  if (!afe->op_set) {
    memcpy(buf, fsm_resp.params, fsm_resp.size);
    afe->param_size = fsm_resp.size;
...
int fsm_afe_save_re25(struct fsadsp_cmd_re25 *cmd_re25) {
  ...
  for (dev = 0; dev < cmd_re25->ndev; dev++) {
    ...
    pr_info("re25.%d[%X]:%d", index, fsm_dev->pos_mask, fsm_dev->re25);
  }
...
int fsm_afe_send_preset(char *preset) {
  ...
  afe.op_set = true;
  ret =
      fsm_afe_send_apr(&afe, (void *)firmware->data, (uint32_t)firmware->size);
  if (ret) {
    pr_err("send apr failed:%d", ret);
  }
...
static const char *fsm_afe_switch_text[] = {"Off", "On"};

static const struct soc_enum fsm_afe_switch_enum[] = {
    SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(fsm_afe_switch_text), fsm_afe_switch_text)};
