...
static ssize_t fsm_misc_read(struct file *filp, char __user *buf, size_t count,
                             loff_t *offset) {
  fsm_config_t *cfg = fsm_get_config();
  struct fsm_misc *fsm_misc;
  struct fsm_dev *fsm_dev;
  ...
  if (ret == count) {
    ...
    pr_err("%02X: reading %zu bytes failed:%d", fsm_misc->addr, count, ret);
  }
  fsm_free_mem((void **)&tmp);
...
static ssize_t fsm_misc_write(struct file *filp, const char __user *buf,
  ...
  if (ret != count) {
    pr_err("%02X: writing %zu bytes failed:%d", fsm_misc->addr, count, ret);
  }
  kfree(tmp);
