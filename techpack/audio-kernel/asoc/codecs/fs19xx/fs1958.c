...
static const struct fsm_srate g_fs19xx_srate_tbl[] = {
    {8000, 0x1},  {16000, 0x3}, {32000, 0x7},  {44100, 0x8},  {48000, 0x9},
    {88200, 0xA}, {96000, 0xB}, {176400, 0xC}, {192000, 0xD},
};
...
int fs19xx_start_up(fsm_dev_t *fsm_dev) {
  ...
  if ((fsm_dev->cur_scene == FSM_SCENE_RCV) ||
      (fsm_dev->cur_scene == FSM_SCENE_TOP_LEFT_BYPASS) ||
      (fsm_dev->cur_scene == FSM_SCENE_TOP_RIGHT_BYPASS) ||
      (fsm_dev->cur_scene == FSM_SCENE_BOT_LEFT_BYPASS) ||
      (fsm_dev->cur_scene == FSM_SCENE_BOT_RIGHT_BYPASS) ||
      (fsm_dev->cur_scene == FSM_SCENE_MMI_ALL_BYPASS) ||
      ((fsm_dev->cur_scene == FSM_SCENE_VOIP_ULTRA) &&
