...
void fsm_mutex_lock(void) { mutex_lock(&g_fsm_mutex); }

void fsm_mutex_unlock(void) { mutex_unlock(&g_fsm_mutex); }
...
int fsm_i2c_bulkwrite(fsm_dev_t *fsm_dev, uint8_t reg, uint8_t *data, int len) {
  uint8_t retries = 0;
  uint8_t *buf;
  int size;
  int ret;
...
struct device *fsm_get_pdev(void) { return g_fsm_pdev; }
...
int fsm_set_monitor(fsm_dev_t *fsm_dev, bool enable) {
  ...
  if (enable) {
    queue_delayed_work(fsm_dev->fsm_wq, &fsm_dev->monitor_work, 5 * HZ);
  } else {
...
static int fsm_request_irq(fsm_dev_t *fsm_dev) {
  ...
  ret = devm_request_threaded_irq(&i2c->dev, fsm_dev->irq_id, NULL,
                                  fsm_irq_hander, irq_flags, "fs19xx", fsm_dev);
  if (ret) {
...
static int fsm_parse_dts(struct i2c_client *i2c, fsm_dev_t *fsm_dev) {
  ...
  if (gpio_is_valid(spksw_gpio)) {
    ret = devm_gpio_request_one(&i2c->dev, spksw_gpio, GPIOF_OUT_INIT_LOW,
                                "FS19XX_SPKSW");
    if (ret) {
...
#ifndef CONFIG_FSM_MTK
static const struct i2c_device_id fsm_i2c_id[] = {{"fs16xx", 0}, {}};
...
int fsm_i2c_init(void) { return i2c_add_driver(&fsm_i2c_driver); }

void fsm_i2c_exit(void) { i2c_del_driver(&fsm_i2c_driver); }
...
static void __exit fsm_mod_exit(void) { fsm_i2c_exit(); }
