...
static ssize_t fsm_re25_show(struct class *class, struct class_attribute *attr,
                             char *buf) {
...
static ssize_t fsm_f0_show(struct class *class, struct class_attribute *attr,
                           char *buf) {
  ...
  for (i = 0, size = 0; i < pfile->hdr.ndev; i++) {
    f0 = (fsm_check_dev_index(pfile->hdr.ndev, i) == 0) ? payload[3 + 6 * i]
                                                        : -65535;
...
static ssize_t fsm_info_show(struct class *class, struct class_attribute *attr,
                             char *buf) {
...
static struct class_attribute g_fsm_class_attrs[] = {
    ...
    __ATTR(fsm_debug, S_IWUSR, NULL, fsm_debug_store), __ATTR_NULL};
